...
const OPENAI_TOOL_DECLARATIONS = LIVE_OPENAI_TOOLS.map((t) => ({
...
// ============================================================================
function parseAndValidateToolCall(toolCall: OpenAI.Chat.ChatCompletionMessageToolCall): {
  toolName: string;
...
// ============================================================================
function parseStructuredRca(content: string | null | undefined): StructuredRca | null {
  ...
  try {
    ...
    logger.warn({ errors: validation.error.errors }, "[Agent] Turn 2 RCA Zod validation failed");
    return null;
  } catch {
    logger.warn({ content: stripped }, "[Agent] Turn 2 RCA JSON parse failed");
    return null;
  }
}
...
// ============================================================================
export const MAX_REINVESTIGATION_ATTEMPTS = 1;
 ...
 */
export async function collectFreshInvestigationEvidence(): Promise<string> {
...
export async function verifyIncidentRecovery(
  ...
  serviceToVerify?: string
): Promise<IncidentVerificationResult> {
  const targetService =
    serviceToVerify === "sentinel-db" || serviceToVerify === "dummy-api"
      ? serviceToVerify
      : "dummy-api";

  incidentService.logEvent(
    `🔍 [Verification Started] Executing explicit closed-loop recovery check for '${targetService}'`
  );
  incidentService.addAiReasoning(
    `Executing explicit closed-loop recovery check for '${targetService}' via verify_recovery...`,
    `verify_recovery(service='${targetService}')`
  );
  emitSentinelEvent("agent.phase_changed", { phase: "VERIFYING", incident_id: incident.id });
  emitSentinelEvent("verification.started", { incident_id: incident.id, service: targetService });

  const verifyExecResult = await executeToolWithGuardrail("verify_recovery", incident.id, {
  ...
  let evidence: string[] = [];
  const rawResult =
    typeof verifyExecResult.result === "string"
  ...
  if (verified) {
    ...
    void safePersist("IncidentRepository", "update", incident.id, () =>
    ...
    incidentService.logEvent(
      `🎉 [Verification Passed] Service '${targetService}' confirmed healthy. Recovery verified in ${elapsedSec}s.`
    );
    incidentService.addAiReasoning(
      `Recovery VERIFIED for '${targetService}': ${evidence.join("; ")}`,
      "verify_recovery",
      "PASSED",
      {
    ...
    emitSentinelEvent("verification.passed", {
      incident_id: incident.id,
      service: targetService,
      evidence,
      recovery_time: incident.recovery_time,
    });
    emitSentinelEvent("service.recovered", {
      recovery_time: incident.recovery_time,
      incident_id: incident.id,
    });
    ...
    incidentService.logEvent(
      `❌ [Verification Failed] Recovery check failed for '${targetService}': ${evidence.join("; ")}`,
      "WARNING"
    );
    incidentService.addAiReasoning(
      `Recovery verification FAILED for '${targetService}'. Service is NOT healthy. Incident remains active.`,
      "verify_recovery",
      "FAILED",
      {
        incident_id: incident.id,
    ...
    emitSentinelEvent("verification.failed", {
      incident_id: incident.id,
      service: targetService,
      evidence,
    });
...
export async function callLlmSreAgent(incident: IncidentData): Promise<void> {
  ...
  incidentService.addAiReasoning(
    "Incident Detected! Starting autonomous two-turn SRE investigation.",
    undefined,
    undefined,
    { incident_id: incidentId, source: "agent" }
  );
  await sleep(400);
...
Investigate this incident. Select the most appropriate Sentinel diagnostic tool to gather evidence before forming a root cause analysis.`;
    ...
    // -----------------------------------------------------------------------
    incidentService.logEvent(
      `🤖 [Turn 1] Invoking OpenAI model '${env.OPENAI_MODEL}' for investigation tool selection (timeout: ${timeoutMs}ms)`
    );
    incidentService.addAiReasoning(
      "Turn 1: Asking AI to select investigation tool...",
      undefined,
      undefined,
      { incident_id: incidentId, turn: 1, source: "openai" }
    );

    const turn1Controller = new AbortController();
    const turn1TimeoutId = setTimeout(() => turn1Controller.abort(), timeoutMs);

    let turn1ToolName: string | null = null;
    let turn1ToolCallId: string = "";
    let turn1RawToolCallMessage: OpenAI.Chat.ChatCompletionMessage | null = null;
    let turn1RealToolResult: string = "";
    ...
    try {
      ...
      const { toolName, validatedArgs, toolCallId } = parseAndValidateToolCall(
        turn1RawToolCallMessage.tool_calls[0]
      );
      turn1ToolName = toolName;
      turn1ToolCallId = toolCallId;

      incidentService.logEvent(
        `🔍 [Turn 1] Model selected investigation tool '${toolName}' with args: ${JSON.stringify(validatedArgs)}`
      );
      incidentService.addAiReasoning(
        `Turn 1: AI selected investigation tool '${toolName}'`,
      ...
      incidentService.logEvent(`⚙️ [Turn 1] Executing '${toolName}' via Safety/Policy Engine`);
      emitSentinelEvent("tool.called", { tool_name: toolName, kwargs: validatedArgs, turn: 1 });

      const toolExecutionResult = await executeToolWithGuardrail(toolName, incidentId, validatedArgs);

      if (toolExecutionResult.status !== "SUCCESS") {
        const errDetail =
          toolExecutionResult.status === "Requires Human Approval"
            ? "Requires Human Approval"
            : toolExecutionResult.message || "Unknown error";
        incidentService.logEvent(
          `⚠️ [Turn 1] Tool '${toolName}' did not complete successfully: ${errDetail}`,
          "WARNING"
        );
        incidentService.addAiReasoning(
          `Turn 1: Investigation tool '${toolName}' blocked/failed (${errDetail}). Activating deterministic fallback.`,
          undefined,
          undefined,
          { incident_id: incidentId, turn: 1, source: "agent" }
        );
      ...
      incidentService.logEvent(
        `✅ [Turn 1] Tool '${toolName}' completed. Result length: ${turn1RealToolResult.length} chars`
      );
      incidentService.addAiReasoning(
        `Turn 1: Tool '${toolName}' executed successfully. Evidence captured.`,
        undefined,
        turn1RealToolResult.slice(0, 300),
        { incident_id: incidentId, turn: 1, source: "openai" }
      );
      emitSentinelEvent("tool.result", { tool_name: toolName, result: turn1RealToolResult, turn: 1 });
      ...
      if (isAbort) {
        incidentService.logEvent(
          `⏱️ [Turn 1 Timeout] OpenAI request timed out after ${timeoutMs}ms. Activating deterministic fallback.`,
          "WARNING"
        );
        incidentService.addAiReasoning(
          "Turn 1: AI investigation timed out (8000ms). Activating deterministic safety fallback.",
          undefined,
          undefined,
          { incident_id: incidentId, turn: 1, source: "agent" }
        );
        ...
        incidentService.logEvent(
          `⚠️ [Turn 1 Error] ${errorMsg}. Activating deterministic safety fallback.`,
          "WARNING"
        );
        incidentService.addAiReasoning(
          `Turn 1: AI call failed (${errorMsg.slice(0, 120)}). Activating deterministic safety fallback.`,
          undefined,
          undefined,
          { incident_id: incidentId, turn: 1, source: "agent" }
        );
    ...
    if (turn1ToolName && turn1RawToolCallMessage && turn1RealToolResult) {
      ...
      incidentService.logEvent(
        `🤖 [Turn 2] Sending conversation with tool evidence back to '${env.OPENAI_MODEL}' for RCA (timeout: ${timeoutMs}ms)`
      );
      incidentService.addAiReasoning(
        "Turn 2: Sending tool evidence to AI for structured root cause analysis...",
        undefined,
        undefined,
        { incident_id: incidentId, turn: 2, source: "openai" }
      );
      ...
      try {
        ...
        incidentService.logEvent(`📋 [Turn 2] Received response. Parsing structured RCA...`);
        ...
        if (!rca) {
          ...
          incidentService.logEvent(
            `⚠️ [Turn 2] Structured RCA parsing/validation failed. Content preview: "${preview}". Activating deterministic fallback.`,
            "WARNING"
          );
          incidentService.addAiReasoning(
            `Turn 2: AI returned malformed RCA. Activating deterministic safety fallback.`
          );
        } else {
          ...
          void safePersist("IncidentRepository", "update", incidentId, () =>
            IncidentRepository.update(incidentId, {
          ...
          incidentService.logEvent(
            `✅ [Turn 2] Structured RCA validated. Root cause: "${rca.root_cause}" (confidence: ${rca.confidence})`
          );
          incidentService.addAiReasoning(
            `Turn 2 RCA: ${rca.root_cause} (Confidence: ${Math.round(rca.confidence * 100)}%)`,
            `Proposed: ${rca.proposed_action.tool}(${JSON.stringify(rca.proposed_action.arguments)})`,
            undefined,
            {
          ...
          emitSentinelEvent("agent.rca_ready", {
            incident_id: incidentId,
            rca,
          });
          ...
          incidentService.logEvent(
            `🛡️ [Guardrail] Routing proposed action '${rca.proposed_action.tool}' through Safety/Policy Engine`
          );

          const remediationResult = await executeToolWithGuardrail(
          ...
          if (remediationResult.status === "Requires Human Approval") {
            incidentService.addAiReasoning(
              `Proposed action '${rca.proposed_action.tool}' blocked by Guardrail Policy Engine. Pending human operator sign-off.`,
              undefined,
            ...
            void safePersist("IncidentRepository", "update", incidentId, () =>
              IncidentRepository.update(incidentId, { status: "REQUIRES_APPROVAL" })
            );

            emitSentinelEvent("incident.updated", incident);
          } else if (remediationResult.status === "SUCCESS") {
            incidentService.addAiReasoning(
              `Remediation '${rca.proposed_action.tool}' executed successfully via safety policy engine. Starting recovery verification...`,
              undefined,
              typeof remediationResult.result === "string"
            ...
            // STEP 2: Persist status update
            void safePersist("IncidentRepository", "update", incidentId, () =>
              IncidentRepository.update(incidentId, { status: "MITIGATING" })
            );
            ...
            // ===============================================================
            const targetService =
              (rca.proposed_action.arguments?.service as string) ||
              (rca.proposed_action.arguments?.container_name as string) ||
              "dummy-api";
            ...
            if (!verifyResult.verified) {
              if ((incident.reinvestigation_attempts || 0) < MAX_REINVESTIGATION_ATTEMPTS) {
                incident.reinvestigation_attempts = (incident.reinvestigation_attempts || 0) + 1;
                incidentService.logEvent(
                  `🔄 [Re-Investigation Started] Recovery verification failed. Initiating bounded re-investigation (attempt ${incident.reinvestigation_attempts}/${MAX_REINVESTIGATION_ATTEMPTS}) with fresh evidence.`
                );
                incidentService.addAiReasoning(
                  `Recovery verification failed. Initiating bounded re-investigation (attempt ${incident.reinvestigation_attempts}/${MAX_REINVESTIGATION_ATTEMPTS}) with fresh evidence.`
                );
                emitSentinelEvent("agent.phase_changed", { phase: "RE_INVESTIGATING", incident_id: incidentId });

                // Collect fresh evidence from running environment
                const freshEvidence = await collectFreshInvestigationEvidence();
                incidentService.logEvent(
                  `📊 [Fresh Evidence] Collected updated system telemetry, service logs, and database status`
                );
...
If an alternate remediation is warranted (e.g. rollback_service), propose it. Reply ONLY with valid JSON.`,
                ...
                incidentService.logEvent(
                  `🤖 [Turn 3] Sending fresh evidence to '${env.OPENAI_MODEL}' for re-investigation RCA (timeout: ${timeoutMs}ms)`
                );
                incidentService.addAiReasoning(
                  "Turn 3: Requesting new RCA from AI using fresh telemetry evidence..."
                );
                ...
                try {
                  ...
                  if (reRca) {
                    ...
                    incidentService.logEvent(
                      `✅ [Turn 3] Re-investigation RCA validated: "${reRca.root_cause}" (confidence: ${reRca.confidence}). Proposing: ${reRca.proposed_action.tool}`
                    );
                    incidentService.addAiReasoning(
                      `Turn 3 RCA: ${reRca.root_cause} (Confidence: ${Math.round(reRca.confidence * 100)}%)`,
                      `Proposed: ${reRca.proposed_action.tool}(${JSON.stringify(reRca.proposed_action.arguments)})`,
                      undefined,
                      {
                        incident_id: incidentId,
                        turn: 3,
                    ...
                    incidentService.logEvent(
                      `🛡️ [Guardrail] Routing re-investigation action '${reRca.proposed_action.tool}' through Safety/Policy Engine`
                    );
                    const reRemediationResult = await executeToolWithGuardrail(
                      reRca.proposed_action.tool,
                      incidentId,
                      reRca.proposed_action.arguments
                    );

                    if (reRemediationResult.status === "Requires Human Approval") {
                      incidentService.addAiReasoning(
                        `Re-investigation proposed action '${reRca.proposed_action.tool}' blocked by Guardrail Policy Engine. Pending human operator sign-off.`,
                        undefined,
                        "Blocked (Requires Human Approval)",
                        {
                          incident_id: incidentId,
                      ...
                      incidentService.addAiReasoning(
                        `Re-investigation action '${reRca.proposed_action.tool}' executed successfully. Verifying recovery...`,
                        undefined,
                        typeof reRemediationResult.result === "string"
                      ...
                      const reVerifyTarget =
                        (reRca.proposed_action.arguments?.service as string) ||
                        (reRca.proposed_action.arguments?.container_name as string) ||
                        "dummy-api";
                      const reServiceToVerify =
                        reVerifyTarget === "sentinel-db" || reVerifyTarget === "dummy-api"
                          ? reVerifyTarget
                          : "dummy-api";
                      ...
                      if (!secondVerify.verified) {
                        incidentService.logEvent(
                          `⛔ [Re-Investigation Limit Reached] Second verification failed. Max attempts reached. Incident remains ESCALATED.`,
                          "ERROR"
                        );
                        incidentService.addAiReasoning(
                          `Re-investigation remediation failed verification. Retry limit (${MAX_REINVESTIGATION_ATTEMPTS}) reached. Escalating incident.`,
                          undefined,
                          undefined,
                          { incident_id: incidentId, turn: 3, source: "agent" }
                        );
                      ...
                      incidentService.addAiReasoning(
                        `Re-investigation remediation '${reRca.proposed_action.tool}' failed: ${reRemediationResult.message ?? "Unknown error"}`,
                        undefined,
                        undefined,
                        { incident_id: incidentId, turn: 3, source: "agent" }
                      );
                    ...
                    incidentService.logEvent(
                      `⚠️ [Turn 3] Re-investigation RCA parsing failed. Escalating incident.`,
                      "WARNING"
                    );
                    incident.status = "ESCALATED";
                ...
                } catch (turn3Err: unknown) {
                  clearTimeout(turn3TimeoutId);
                  incidentService.logEvent(
                    `⚠️ [Turn 3 Error] ${String(turn3Err)}. Escalating incident.`,
                    "WARNING"
                  );
                  incident.status = "ESCALATED";
                ...
                incidentService.logEvent(
                  `⛔ [Re-Investigation Limit Reached] Attempts (${incident.reinvestigation_attempts}) reached max (${MAX_REINVESTIGATION_ATTEMPTS}). Escalating incident.`,
                  "ERROR"
                );
                incident.status = "ESCALATED";
            ...
            incidentService.addAiReasoning(
              `Remediation '${rca.proposed_action.tool}' failed: ${remediationResult.message ?? "Unknown error"}`,
              undefined,
              undefined,
              { incident_id: incidentId, source: "agent" }
            );
        ...
        if (isAbort) {
          incidentService.logEvent(
            `⏱️ [Turn 2 Timeout] OpenAI Turn 2 timed out after ${timeoutMs}ms. Activating deterministic fallback.`,
            "WARNING"
          );
          incidentService.addAiReasoning(
            "Turn 2: AI RCA generation timed out (8000ms). Activating deterministic safety fallback."
          );
        } else {
          const errorMsg = err instanceof Error ? err.message : String(err);
          incidentService.logEvent(
            `⚠️ [Turn 2 Error] ${errorMsg}. Activating deterministic fallback.`,
            "WARNING"
          );
          incidentService.addAiReasoning(
            `Turn 2: AI RCA failed (${errorMsg.slice(0, 120)}). Activating deterministic safety fallback.`
          );
        }
      }
    }
  ...
  if (!aiInvestigationSucceeded) {
    ...
    incidentService.addAiReasoning(
      "Root cause: PostgreSQL database connection refused (Deterministic SRE rule)",
      undefined,
      undefined,
      { incident_id: incidentId, source: "deterministic" }
    );
    await sleep(400);
    ...
    if (remediation.status === "Requires Human Approval") {
      incidentService.addAiReasoning(
        ...
        undefined,
        "Blocked (Requires Human Approval)",
        { incident_id: incidentId, source: "guardrail" }
      );
      incidentService.setSystemHealth("INCIDENT_ACTIVE");
      incident.status = "REQUIRES_APPROVAL";
      ...
      incidentService.addAiReasoning(
        "Database service restart command dispatched successfully via guardrail. Verifying recovery...",
        undefined,
      ...
      if (!verifyRes.verified) {
        if ((incident.reinvestigation_attempts || 0) < MAX_REINVESTIGATION_ATTEMPTS) {
          incident.reinvestigation_attempts = (incident.reinvestigation_attempts || 0) + 1;
          incidentService.logEvent(
            `🔄 [Deterministic Re-Investigation] Verification failed for sentinel-db. Proposing rollback_service via guardrail.`
          );
          incidentService.addAiReasoning(
            `Deterministic re-investigation: restart_service failed recovery verification. Proposing rollback_service for operator authorization.`,
            "rollback_service(service='sentinel-db')",
            undefined,
            { incident_id: incidentId, source: "deterministic" }
          );

          // Route through guardrail -> requires human approval
          const rollbackResult = await executeToolWithGuardrail("rollback_service", incidentId, {
            service: "sentinel-db",
          });
          ...
          incidentService.logEvent(
            `⛔ [Deterministic Engine] Max re-investigation attempts reached. Incident remains ESCALATED.`,
            "ERROR"
          );
          incident.status = "ESCALATED";
